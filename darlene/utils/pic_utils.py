...
from .darlene_logger import logger
from .file_utils import check_dir

from darlene.conf.basic import cache_path

img_temp_path = cache_path / "img/"
'''图片文件临时目录'''

check_dir(img_temp_path)
...
async def get_img(url: str) -> Optional[bytes]:
    ...
    try:
        ...
        logger.error(f"获取图片时出错，错误原因 【{e}】，图片链接为 【{url}】。")
        return None


def save_img(img: bytes, path: Union[str,Path] = img_temp_path):
    '''
    保存bytes变量中的图片。
    :param img: 需要保存的图片
    :param path: 保存图片的路径
