...
@apply.handle()
async def apply_handle_first_recieve(bot: Bot,event: GroupMessageEvent,state: T_State,args: Message = CommandArg()):
    """
    申请投票 类型（踢出/禁言） @某人
    """
...
async def _(bot: Bot, event: GroupMessageEvent, type: Message = Arg(), person: Message = Arg()):
    if person["type"] != "at":
